const CONVERTER_CACHE_VERSION = "converter-webapp-v2";
...
const CONVERTER_SHELL_URLS = [
    "/",
    "/css/tools/fonts.css?v2.2.0",
    "/css/tools/converter/reset.css?v4.0.0",
    "/css/tools/converter/main.css?v5.2.23",
    "/js/tools/converter/utils.js?v5.1.4",
    "/js/tools/converter/websocket.js?v5.1.11",
...
function putInCache(request, response) {
    ...
    caches.open(CONVERTER_CACHE_VERSION).then(function (cache) {
...
function precacheShell(cache) {
    return Promise.all(
        CONVERTER_SHELL_URLS.map(function (url) {
...
self.addEventListener("install", function (event) {
    event.waitUntil(
        caches.open(CONVERTER_CACHE_VERSION).then(precacheShell).then(function () {
    ...
    event.waitUntil(
        caches.keys().then(function (keys) {
            return Promise.all(
                keys
                    .filter(function (key) {
                        return key.startsWith("converter-webapp-") && key !== CONVERTER_CACHE_VERSION;
                    })
                    .map(function (key) {
