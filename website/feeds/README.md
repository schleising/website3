...
96. The All Feeds reader view shall be treated as the navigation root/end-point for feed-category history mode; when the user is on All Feeds, no additional in-app Back target shall be introduced by category-link handling **or by history `pushState` boundary entries**. On All Feeds, Back once shall leave/close the app (or the previous non-feeds page in a normal browser tab).
